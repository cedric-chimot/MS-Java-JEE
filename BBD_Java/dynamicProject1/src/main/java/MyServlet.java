...
public class MyServlet extends HttpServlet {
	...
	private void ajoutCommandeEtLignes(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    while (parameterNames.hasMoreElements()) {
	        ...
	        if (paramName.startsWith("article") && paramName.length() > "article".length()) {
	            int idArticle = Integer.parseInt(paramName.substring("article".length()));
	    ...
	    Connexion co = new Connexion();
	    Connection cn = null; // Initialiser la connexion en dehors du bloc try-catch
	    int idCommande = -1; // Initialiser idCommande à -1

	    try {
	        cn = co.getDatabaseConnection();
	        cn.setAutoCommit(false); // Début de la transaction

	        // Utiliser la méthode ajouterCommande de la classe Connexion
	        idCommande = co.ajouterCommande(dateCommande, idClient, lignesCommande, cn);

	        // Récupérer l'ID de la commande après l'ajout
	        idCommande = co.recupIdCommande(dateCommande, idClient);
	        ...
	        if (cn != null) {
	            try {
	                cn.rollback(); // Annulation de la transaction en cas d'erreur
	        ...
	        if (cn != null) {
	            try {
	                cn.setAutoCommit(true); // Rétablissement du mode de traitement par défaut
	    ...
	    // Utiliser idCommande comme nécessaire dans le reste de votre code
